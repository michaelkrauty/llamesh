...
wedge_detector:
  ...
  cpu_only: false                        # assert no GPU of any vendor; only then is idle CPU a wedge (default: false)
...
* `created` is a stable Unix timestamp (seconds), not the request time, so a given model reports the same value across requests. For locally served models it is this node's start time; for models advertised by cluster peers it is the owning peer's start time (carried in gossip), so the value is consistent across every front-end node that serves the model. Peers that do not advertise a start time (older versions) fall back to the querying node's start time.
* Metadata includes `model`, `profile_id`, `profile_description`, `idle_timeout_seconds`, `estimated_tokens_per_second` (from learned metrics), and `parsed_model_params` (an object extracted from the instance startup log). Parsed params are persisted per `args_hash` in the metrics snapshot when an instance becomes ready, so they remain available after the instance is evicted and across proxy restarts; a running instance's values take precedence. The llama.cpp version is recorded alongside, and persisted params observed under a different binary than the one currently live are withheld until the next instance startup refreshes them. The field is `null` only for profiles that have never had a ready instance on this node (or whose launch args — and therefore hash — changed since, or whose params predate a llama.cpp upgrade). At default llama.cpp log verbosity it carries the effective context window (`n_ctx`) and the resolved slot count (`n_slots`), both read from the instance's slot-init line. The trained context length (`n_ctx_train`) is present only when the requested context exceeds it, since that is the only case llama.cpp reports it at this verbosity (in a capacity warning); the opposite case is reported at a level this verbosity hides. The model-metadata fields (architecture, layer/head counts, etc.) are additionally populated on older builds or when the instance is launched at raised verbosity, and are `null` otherwise.
...
* **Per model:profile** — the `proxy_hash_*` family (`requests_total`, `errors_total`, `tokens_generated_total`, `total_latency_ms`, latency percentiles, `peak_vram_mb`, `peak_sysmem_mb`), labeled by the profile's args hash and comma-separated display names. Names are sorted lexically so an unchanged name set retains the same label value across restarts.
...
* Startup log parsing:
  ...
  * The proxy parses `llama-server` startup logs to extract model parameters. At default log verbosity it reads the effective context window (`n_ctx`) and the resolved parallel-slot count (`n_slots`) from the slot-init line, plus the trained context length (`n_ctx_train`) when llama.cpp reports it in a capacity warning, which it does only when the requested context exceeds the trained one; the model-metadata block (`n_layer`, architecture, etc.) is also parsed when present (older builds or raised verbosity). Both the current and the historical spellings of these lines are accepted, so a pinned older `llama.cpp` keeps parsing. These are exposed in `/v1/models` metadata as `parsed_model_params`.
...
* Wedged-instance watchdog (`wedge_detector`, opt-in, disabled by default):
  ...
  * A request whose `llama-server` accepts it then goes silent (the connection stays open but no response bytes arrive) keeps `in_flight_requests > 0` forever. That instance is never idle-eligible (idle eviction requires `in_flight_requests == 0`) and the held slot keeps `current_requests > 0`, so a graceful drain (`wait_for_drain`) cannot complete until the process is killed by hand. `upstream_read_timeout_ms` bounds this per request — for every request to a local `llama-server`, and, on the same setting, for the response body of a *streaming* request forwarded to a cluster peer over the Noise transport (each body chunk is read under a per-chunk inactivity timeout), so a peer that goes silent mid-stream cannot pin the forwarding node's slot either. A *non-streaming* peer forward buffers the peer body and retries on read failure (re-forwarding), so it is intentionally not bounded by this timeout; the watchdog is the instance-level complement (local instances only).
  ...
  * Streaming backpressure: for a `stream: true` request, the slot is held while the *downstream client* consumes the body. A client that consumes slowly keeps triggering generation, so the instance is not flat and is never flagged. Only a streaming client that consumes *zero* bytes for the entire `window_ms` (an abandoned/dead consumer) leaves the instance flat long enough to be killed. Because most streaming responses complete in well under `window_ms`, and any active consumer resets the streak, this is a narrow edge; deployments with very long-lived, very slowly-drained streams should raise `window_ms` accordingly. Distinguishing a stalled upstream from a fully-stalled downstream consumer without killing the latter is tracked as a follow-up.
...
When direct gossip advertises a loopback placeholder, the receiver can infer a routing endpoint from the connection's source IP and the advertised listener port and scheme. Locally inferred endpoints follow subsequent direct gossip when those values change; the ephemeral TCP source port is never used. Same-host peers can refresh loopback endpoints, but a loopback source cannot replace an existing non-loopback route. Missing or unusable source information preserves the prior endpoint and its local provenance. A current non-loopback advertisement takes precedence, and previously advertised, configured, or transitively learned non-loopback routes remain protected from source-based replacement. Configured seed matching uses the node ID or an equivalent existing URL without DNS alias inference. Provenance is never serialized or accepted from another node; raw mDNS records remain discovery candidates rather than routing authority. Recovery of stale transitive routes or arbitrary seed aliases requires separate identity-bound endpoint reconciliation.
...
* **Single-port multiplexing**: Protocol detection (`protocol_detect.rs`) distinguishes TLS, HTTP/1.1, prior-knowledge cleartext HTTP/2 (h2c), and Noise connections on the same port. Detection runs independently per connection under `http.protocol_detect_timeout_ms`, so a silent client or partial prefix does not block admission of other connections. Shutdown cancels pending detection and prevents dispatch once draining is observed. No separate port is needed for inter-node traffic. h2c connections are served by the HTTP/2 stack; their lifetime is bounded by ping-based keep-alive plus an idle watchdog that gracefully shuts down (GOAWAY) connections with no active streams for `http.idle_timeout_seconds` and drops connections that cannot complete a graceful shutdown (e.g. a stalled handshake). TLS does not advertise ALPN, so TLS clients negotiate HTTP/1.1.
