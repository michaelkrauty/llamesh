...
* `created` is the node's start time (a Unix timestamp in seconds) and is stable for the lifetime of the process; it is not the request time, so a given model reports the same value across requests.
